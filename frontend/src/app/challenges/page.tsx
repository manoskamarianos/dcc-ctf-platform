...
const getDifficultyIcon = (diff: string) => {
    switch (diff.toLowerCase()) {
        case "easy": return <Zap className="w-3 h-3 mr-1" />;
        case "medium": return <Cpu className="w-3 h-3 mr-1" />;
        case "hard": return <AlertCircle className="w-3 h-3 mr-1" />;
        case "insane": return <Skull className="w-3 h-3 mr-1" />;
        default: return null;
    }
};
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {challenges.length === 0 && (
                <div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-500 border border-dashed border-gray-800 rounded-md bg-black/50">
                    ...
                    <p className="turret-light text-sm">Sector clear. No active challenges.</p>
                </div>
            )}
...
function ChallengeCard({ challenge }: { challenge: Challenge }) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => {
            setIsOpen(open);
            if(!open) setStatus(null); // Reset status on close
        }}>
            ...
            <DialogContent className="bg-black/95 border border-terminal-green text-white max-w-2xl shadow-[0_0_50px_rgba(34,197,94,0.15)] p-0 gap-0">
                {/* Terminal Header */}
                <DialogHeader className="bg-gray-900/50 border-b border-gray-800 p-4">
                    <div className="flex items-center justify-between">
                    ...
                    <DialogDescription className="text-gray-500 font-mono text-xs flex gap-3 pt-1">
                        ...
                        <span>//</span>
                        <span className="text-white">{challenge.points} PTS</span>
                        <span>//</span>
                        <span className="uppercase text-white">{challenge.category}</span>
                    </DialogDescription>
                </DialogHeader>
                    ...
                    {!challenge.solved ? (
                        ...
                        >
                            <div className="space-y-3">
                                ...
                                >
                                    <span className="animate-pulse">_</span> INPUT_FLAG
                                </Label>
